...
if (isset($_POST['display_name']) && !empty(secure($_POST['display_name'])) && $_POST['display_name'] != $user->name()) {
    ...
    if ($database->query("SELECT `display_name` FROM users WHERE display_name = :display_name AND id != :id", [':display_name' => $_POST['display_name'], ':id' => $user->id()])->rowCount() > 0) toastr('error', 'Name already taken');
    $info['display_name'] = secure($_POST['display_name']);
...
if (isset($_POST['username']) && !empty(secure($_POST['username'])) && $_POST['username'] != $user->username()) {
    ...
    if ($database->query("SELECT `name` FROM users WHERE `name` = :username AND id != :id", [':username' => strtolower(secure($_POST['username'])), ':id' => $user->id()])->rowCount() > 0) toastr('error', 'Username already taken');
    $info['name'] = strtolower(secure($_POST['username']));
...
if (isset($_POST['email']) && !empty(secure($_POST['email'])) && $_POST['email'] != $user->email()) {
    ...
    if ($database->query("SELECT `email` FROM users WHERE email = :email AND id != :id", [':email' => $_POST['email'], ':id' => $user->id()])->rowCount() > 0) toastr('error', 'Email already taken');
