...
class Logs extends User {
    ...
    public function getLogs() {
        return $this->query("SELECT logs.*, users.id, users.name, users.avatar, roles.role_title FROM logs INNER JOIN users ON users.id = logs.user_id INNER JOIN user_roles ON user_roles.user_id = users.id INNER JOIN roles ON roles.role_id = user_roles.role_id")->fetchAll();
    }
